...
const Sell = ({
   ...
   const increment = (id) => {
      ...
      const alreadyExists = productSells.find((item) => item.id === id);
      ...
      console.log(productSells); // Ini akan menampilkan state sebelum diperbarui
   };
   const decrement = (id) => {
      const productIndex = productSells.findIndex((item) => item.id === id);
      ...
      <>
         ...
         <div className="flex flex-wrap gap-3">
            {!isLoading &&
               ...
               data.map((item, i) => {
                  const qty =
                     productSells.find((prod) => prod.id === item.id)?.qty ?? 0;
                  return (
                     <ProductCard
                        item={item}
                        getAll={getAll}
